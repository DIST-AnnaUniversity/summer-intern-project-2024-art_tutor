...
def get_dominant_colors(image_path, num_colors=10):
    ...
    # Fit K-Means model to extract dominant colors
...
def plot_color_checkboxes(colors):
    ...
    box_size = 20  # Size of each color box (checkbox)
    fig_height = num_colors * 0.8 + 1  # Height based on number of colors
    fig, ax = plt.subplots(figsize=(8, fig_height))  # Adjust size for checkboxes
    ...
    ax.set_xlim(0, box_size * 2.5)
    ax.set_ylim(0, num_colors)
    ax.axis('off')
    plt.show()

# Example usage
dominant_colors = get_dominant_colors('static/27.jpeg', num_colors=10)
plot_color_checkboxes(dominant_colors)
