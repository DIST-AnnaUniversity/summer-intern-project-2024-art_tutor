...
# Display layers with feedback
def display_layers(image, layers, feedback):
    plt.figure(figsize=(15, 10))
    plt.subplot(1, len(layers) + 1, 1)
    plt.imshow(image)
    plt.title('Original Image')

    for i, (layer, fb) in enumerate(zip(layers, feedback)):
        plt.subplot(1, len(layers) + 1, i + 2)
        plt.imshow(layer)
        plt.title(f'Layer {i}\n{fb}')
...
def upload():
    ...
    # Generate dynamic feedback (dummy feedback for demonstration)
    feedback = [f"Layer {i} processed." for i in range(kmeans.n_clusters)]

    # Display layers
    display_layers(image_rgb, layers, feedback)
